...
const filterDictByKeys = (keys: string[], dict: FreqDict) =>
    Object.keys(dict).reduce(
        (result, key) =>
            keys.indexOf(key) === -1 ? result : { ...result, [key]: dict[key] },
        {} as FreqDict
    );
...
const companyPersonalityFreq = (employees: Employee[]): FreqDict =>
    employees.reduce(
        (freq, emp) => sumFreqs(freq, frequency(emp.personality)),
        {} as FreqDict
    );
 ...
 *
 */
export const employeeHappiness = (hired: Employee[]) => (
    employee: Employee
): number => {
    ...
    const myPersonalitiesRt = filterDictByKeys(
        employee.personality,
        personalityRatios
    );
    const mySpeciesRt = filterDictByKeys([employee.species], speciesRatios);

    const allRatios = Object.values(myPersonalitiesRt).concat(
        Object.values(mySpeciesRt)
    );
...
export const dailyIncome = (hired: Employee[]): number =>
    ...
    hired.reduce(
        (result, emp) =>
            result +
            emp.skills.reduce((result2, skill) => result2 + skill.value, 0),
        0
    );
